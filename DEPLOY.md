...
> ⚠️ **用 Web 选点页面时，坐标不会自动生效，必须按下面顺序操作（实测有效，无需重启）：**
> 1. 设置 → 隐私与安全性 → 定位服务 → **关闭定位服务**
> 2. 在 Web 选点页面选好位置 → 「储存到设备」（显示 ✓）
> 3. **重新打开定位服务**
> 4. 打开地图 → 生效
>
> 原因：Web 页面只把坐标写进存储，不会让 `locationd` 重新定位，它仍用当前会话的旧定位；先关再开定位服务才会强制重新发起 WLOC 请求拿到新坐标。**快捷指令经地图 App 共享触发，顺带重激活了定位流程，所以不需要这步**——这也是「快捷指令直接生效、Web 页面不生效」的真正原因。
