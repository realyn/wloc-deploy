...
> ⚠️ **用 Web 选点页面时，坐标不会自动生效，必须按下面顺序操作（实测有效，无需重启设备）：**
> 1. 打开「设置」→ 隐私与安全性 → 定位服务 → **关闭定位服务**
> 2. **杀掉地图 App 进程**（上划从后台彻底关闭）
> 3. 在 Web 选点页面选好位置 → 「储存到设备」（显示 ✓）
> 4. 打开「设置」→ **重新开启定位服务**
> 5. **再次杀掉地图 App 进程**
> 6. 打开地图 → 生效
>
> 原因：Web 页面只把坐标写进存储，不会让 `locationd` 重新定位。先关定位服务 + 杀掉地图进程结束旧定位会话，再开定位服务 + 再杀一次进程，地图才会重新发起 WLOC 请求并拿到新坐标，避免复用缓存的旧定位。**快捷指令经地图 App 共享触发，顺带重激活了定位流程，所以不需要这些步骤**——这也是「快捷指令直接生效、Web 页面不生效」的真正原因。
